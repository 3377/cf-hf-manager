import { getConfig, getSpaces, parseUserTokenMapping } from '../../../../../helpers';
...
export async function onRequestPost(context) {
  try {
    ...
    // 从用户令牌映射中获取
    const { userTokenMapping } = parseUserTokenMapping(context.env);
    ...
    console.error(`重启Space ${context.params.spaceId} 错误:`, error);
    return new Response(JSON.stringify({
      success: false,
      message: `重启Space失败: ${error.message}`
    }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
